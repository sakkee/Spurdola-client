...
class Scrollable(Wrapper, Controller, ControllerManager):
    def __init__(self, content=None, width=None, height=None, is_fixed_size=False, fs=None):
        if is_fixed_size:
    ...
    def set_manager(self, manager):
        ...
        self.group['background'] = pyglet.graphics.OrderedGroup(20, self.root_group)
        self.group['foreground'] = pyglet.graphics.OrderedGroup(30, self.root_group)
        self.group['highlight'] = pyglet.graphics.OrderedGroup(25, self.root_group)
    ...
    def _load_scrollbars(self, width, height):
        ...
        if self.content.height > height:
            if self._vscrollbar is None:
                self._vscrollbar = VScrollbar(height,fs=self._fs)
    ...
    def on_lose_highlight(self):
        #for item in self.content._content:
        #    if hasattr(item, 'on_lose_highlight'):
        #        item.on_lose_highlight()
