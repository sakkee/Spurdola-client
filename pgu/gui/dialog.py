...
class FileDialog(Dialog):
    ...
    def __init__(self, title_txt="File Browser", button_txt="Okay", cls="dialog", path=None):
        ...
        self.list = area.List(width=350, height=400)
        self.input_dir = input.Input()
        self.input_file = input.Input()
        self._list_dir_()
        self.button_close=button.Button("X")
        ...
        self.body.td(self.button_close,style=td_style,align=1)
        self.body.tr()
        self.body.td(self.list, colspan=3, style=td_style)
        self.list.connect(CHANGE, self._item_select_changed_, None)
        self.button_close.connect(CLICK,self._button_close_clicked_,None)
    ...
    def _button_close_clicked_(self,arg):
        self.close()
