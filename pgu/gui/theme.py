...
class Theme:
    ...
    def __init__(self,dirs='data/themes/default'):
        """Theme constructor.
...
#                 s = w._theme_paint_bkgr.convert()
        ...
        return theme_paint
    def mouseBack(self):
        curs,mask = pygame.cursors.compile(pygame.cursors.taa, '.', 'X')
        ...
        def theme_event(e):
            ...
            if e.type == MOUSEBUTTONUP or e.type == MOUSEBUTTONDOWN:
                if e.type==MOUSEBUTTONDOWN:
                    curs,mask = pygame.cursors.compile(pygame.cursors.taa1, '.', 'X')
                    pygame.mouse.set_cursor((24,32),(0,0),curs,mask)
                    self.t = Timer(1,self.mouseBack)
                    self.t.start();
